document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!player) {
        if (resultBox) {
            ...
            resultBox.innerHTML = "⚠️ ابتدا بازیکن خود را بسازید.";
        }
        return;
    }
    ...
    const getAttr = (name, fallback = 60) =>
        clamp(attrs[name] ?? fallback, 1, 99);
    ...
    const calculatePerformance = () => {
        ...
        const fatigue = Number(player.fatigue ?? 10);

        const attack = (
            getAttr("shooting") +
            getAttr("finishing", 58) +
            getAttr("pace") +
            getAttr("dribbling")
        ) / 4;
        ...
        const creation = (
            getAttr("passing", 62) +
            getAttr("vision", 60) +
            getAttr("ballControl", 65) +
            getAttr("dribbling")
        ) / 4;

        const defense = (
            getAttr("defending", 45) +
            getAttr("tackling", 40) +
            getAttr("interceptions", 38) +
            getAttr("physical")
        ) / 4;
    ...
    const playMatch = () => {
        ...
        const teamGoals = clamp(
            Math.round(1.35 + matchFactor / 18 + (Math.random() * 2 - 1)),
            0,
            5
        );

        const opponentGoals = clamp(
            Math.round(1.25 - matchFactor / 20 + (Math.random() * 2 - 1)),
            0,
            5
        );

        const outcome = teamGoals > opponentGoals
            ? "win"
        ...
        const rating = clamp(
            (6.2 + (performance - opponent) / 13 + Math.random() * 1.5).toFixed(1),
            4,
            10
        );

        const position = player.position || "ST";
        const goalChance = ["ST", "LW", "RW"].includes(position) ? 0.48 :
            ["CAM", "CM"].includes(position) ? 0.25 : 0.10;
        const assistChance = ["CAM", "CM", "LW", "RW"].includes(position) ? 0.42 : 0.20;

        const goals = Math.min(
            teamGoals,
            Math.random() < goalChance
                ? 1 + (Math.random() < Math.max(0.05, (performance - 70) / 150) ? 1 : 0)
                : 0
        );

        const remainingGoals = Math.max(0, teamGoals - goals);
        ...
        player.fanRating = clamp(
            Number(player.fanRating ?? player.popularity ?? 50) +
            fanChange +
            (rating >= 8 ? 2 : rating >= 7 ? 1 : 0)
        );
        player.popularity = player.fanRating;

        player.matchHistory = Array.isArray(player.matchHistory)
            ? player.matchHistory
            : [];

        player.matchHistory.unshift({
            date: new Date().toLocaleDateString("fa-IR"),
            opponent: `تیم رقیب ${opponent}`,
        ...
        set(
            "resultText",
            `${outcome === "win" ? "برد" : outcome === "loss" ? "باخت" : "مساوی"} ${teamGoals} - ${opponentGoals}`
        );
